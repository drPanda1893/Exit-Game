...
public class BuildLevel1PrisonCell : LevelBuilderBase
{
    ...
    private GameObject BuildCellEnvironment(Transform root, GameObject numpadPanel, TextMeshProUGUI hintText)
    {
        ...
        // Trigger hinter der Tür → lädt Level2 wenn Spieler durchläuft
        GameObject triggerGO = new GameObject("LevelTransitionTrigger");
        triggerGO.transform.position = new Vector3(3.2f, 1.2f, 0f);
        triggerGO.transform.SetParent(root);
        BoxCollider triggerCol = triggerGO.AddComponent<BoxCollider>();
        triggerCol.isTrigger = true;
        triggerCol.size      = new Vector3(0.6f, 2.4f, 1.3f);
        triggerGO.AddComponent<LevelTransitionTrigger>().targetScene = "Level2";
